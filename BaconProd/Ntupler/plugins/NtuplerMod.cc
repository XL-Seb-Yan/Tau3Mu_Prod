  ...
  fPFParArr          (0)
{
  ...
  if(iConfig.existsAs<edm::ParameterSet>("Muon",false)) {
    ...
    if(fIsActiveMuon) {
      ...
      fMuonPtErr  = new std::vector<float>(); assert(fMuonPtErr);
      fMuonStaPt  = new std::vector<float>(); assert(fMuonStaPt);
      fMuonStaEta = new std::vector<float>(); assert(fMuonStaEta);
      fMuonStaPhi = new std::vector<float>(); assert(fMuonStaPhi);
      fMuonPfPt   = new std::vector<float>(); assert(fMuonPfPt);
      fMuonPfEta  = new std::vector<float>(); assert(fMuonPfEta);
      fMuonPfPhi  = new std::vector<float>(); assert(fMuonPfPhi);    
      fMuonQ      = new std::vector<int>(); assert(fMuonQ);
      fMuonTrkIso     = new std::vector<float>(); assert(fMuonTrkIso);
      fMuonEcalIso    = new std::vector<float>(); assert(fMuonEcalIso);
      fMuonHcalIso    = new std::vector<float>(); assert(fMuonHcalIso);
      fMuonChHadIso   = new std::vector<float>(); assert(fMuonChHadIso);
      fMuonGammaIso   = new std::vector<float>(); assert(fMuonGammaIso);
      fMuonNeuHadIso  = new std::vector<float>(); assert(fMuonNeuHadIso);
      fMuonPuIso      = new std::vector<float>(); assert(fMuonPuIso);
      fMuonD0         = new std::vector<float>(); assert(fMuonD0);
      fMuonDz         = new std::vector<float>(); assert(fMuonDz);
      fMuonSip3d      = new std::vector<float>(); assert(fMuonSip3d);
      fMuonTkNchi2    = new std::vector<float>(); assert(fMuonTkNchi2);
      fMuonMuNchi2    = new std::vector<float>(); assert(fMuonMuNchi2);
      fMuonTrkKink    = new std::vector<float>(); assert(fMuonTrkKink);
      fMuonGlbKink    = new std::vector<float>(); assert(fMuonGlbKink);
      fMuonNValidHits    = new std::vector<int>(); assert(fMuonNValidHits);
      fMuonTypeBits      = new std::vector<unsigned int>(); assert(fMuonTypeBits);
      fMuonSelectorBits  = new std::vector<unsigned int>(); assert(fMuonSelectorBits);
      fMuonPogIDBits     = new std::vector<unsigned int>(); assert(fMuonPogIDBits);
      fMuonNTkHits       = new std::vector<unsigned int>(); assert(fMuonNTkHits);
      fMuonNPixHits      = new std::vector<unsigned int>(); assert(fMuonNPixHits);
      fMuonNTkLayers     = new std::vector<unsigned int>(); assert(fMuonNTkLayers);
...
NtuplerMod::~NtuplerMod()
{
  ...
  delete fMuonEcalIso;
  delete fMuonHcalIso;
  delete fMuonChHadIso;
  delete fMuonGammaIso;
  delete fMuonNeuHadIso;
  delete fMuonPuIso;
  delete fMuonD0;
  delete fMuonDz;
  delete fMuonSip3d;
  ...
  delete fMuonTrkKink;
  delete fMuonGlbKink;
  delete fMuonNValidHits;
  delete fMuonTypeBits;
  delete fMuonSelectorBits;
  delete fMuonPogIDBits;
  delete fMuonNTkHits;
  delete fMuonNPixHits;
  delete fMuonNTkLayers;
...
void NtuplerMod::beginJob()
{
  ...
  if(fIsActiveMuon)   {
    ...
    fEventTree->Branch("MuonPtErr",   fMuonPtErr);
    fEventTree->Branch("MuonStaPt",   fMuonStaPt);
    fEventTree->Branch("MuonStaEta",   fMuonStaEta);
    fEventTree->Branch("MuonStaPhi",   fMuonStaPhi);
    fEventTree->Branch("MuonPfPt",   fMuonPfPt);
    fEventTree->Branch("MuonPfEta",   fMuonPfEta);
    fEventTree->Branch("MuonPfPhi",   fMuonPfPhi);
    fEventTree->Branch("MuonQ",   fMuonQ);
    fEventTree->Branch("MuonTrkIso",   fMuonTrkIso);
    fEventTree->Branch("MuonEcalIso",   fMuonEcalIso);
    fEventTree->Branch("MuonHcalIso",   fMuonHcalIso);
    fEventTree->Branch("MuonChHadIso",   fMuonChHadIso);
    fEventTree->Branch("MuonGammaIso",   fMuonGammaIso);
    fEventTree->Branch("MuonNeuHadIso",   fMuonNeuHadIso);
    fEventTree->Branch("MuonPuIso",   fMuonPuIso);
    fEventTree->Branch("MuonD0",   fMuonD0);
    fEventTree->Branch("MuonDz",   fMuonDz);
    fEventTree->Branch("MuonSip3d",   fMuonSip3d);
    fEventTree->Branch("MuonTkNchi2",   fMuonTkNchi2);
    fEventTree->Branch("MuonMuNchi2",   fMuonMuNchi2);
    fEventTree->Branch("MuonTrkKink",   fMuonTrkKink);
    fEventTree->Branch("MuonGlbKink",   fMuonGlbKink);
    fEventTree->Branch("MuonNValidHits",   fMuonNValidHits);
    fEventTree->Branch("MuonTypeBits",   fMuonTypeBits);
    fEventTree->Branch("MuonSelectorBits",   fMuonSelectorBits);
    fEventTree->Branch("MuonPogIDBits",   fMuonPogIDBits);
    fEventTree->Branch("MuonNTkHits",   fMuonNTkHits);
    fEventTree->Branch("MuonNPixHits",   fMuonNPixHits);
    fEventTree->Branch("MuonNTkLayers",   fMuonNTkLayers);
...
void NtuplerMod::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(fIsActiveMuon) {
    ...
    fMuonTrkKink->clear();
    fMuonGlbKink->clear();
    fMuonNValidHits->clear();
    fMuonTypeBits->clear();
    fMuonSelectorBits->clear();
    fMuonPogIDBits->clear();
    fMuonNTkHits->clear();
    fMuonNPixHits->clear();
    fMuonNTkLayers->clear();
    ...
    isFill = fFillerMuon->fill(fMuonPt, fMuonEta, fMuonPhi, fMuonPtErr, fMuonStaPt, fMuonStaEta, fMuonStaPhi, fMuonPfPt, fMuonPfEta, fMuonPfPhi, fMuonQ, fMuonTrkIso, fMuonEcalIso, fMuonHcalIso, fMuonChHadIso,
		      ...
		      fMuonGlbKink, fMuonNValidHits, fMuonTypeBits, fMuonSelectorBits, fMuonPogIDBits, fMuonNTkHits, fMuonNPixHits, fMuonNTkLayers, fMuonNPixLayers,
			       fMuonNMatchStn, fMuonTrkID, fMuonHltMatchBits, fVfTc, fVfDof, fVfNc, fVfProb, fCategory, fVfValid, fVfIp, fTriIso, fInvMass, iEvent, iSetup, *pv, fTrigger->fRecords, *hTrgEvt);
  }
